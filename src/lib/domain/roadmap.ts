...
const SEEDS: TaskSeed[] = [
  ...
  /* Грантовый конкурс РК идёт своим циклом и заканчивается раньше основной
     подачи: заявление подаётся сразу после результатов ЕНТ, а не вместе с
     документами в вуз. Отдельная ветка нужна именно поэтому — по общему
     дедлайну программы этот срок уже прошёл бы. */
  {
    key: "grant-apply",
    title: "Подать заявление на конкурс государственного гранта",
    why: "Конкурс грантов идёт отдельным циклом сразу после ЕНТ и закрывается раньше приёма документов в вуз: пропущенный срок переносит грант на следующий год.",
    category: "admin",
    leadMonths: 2.5,
    effortHours: 3,
    when: (_p, pr) => pr.costs.grant !== undefined,
  },
  {
    key: "grant-threshold-check",
    title: "Сверить свой балл ЕНТ с пороговым баллом конкурса",
    why: "Ниже порога заявку на грант не принимают вовсе — это проверяется до сбора документов, а не после.",
    category: "research",
    leadMonths: 3.5,
    effortHours: 1,
    when: (_p, pr) => pr.costs.grant !== undefined,
