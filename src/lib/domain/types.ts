...
/**
 * Грантовый трек Казахстана — отдельный способ поступления, а не разновидность
 * стипендии.
 *
 * В РК грант разыгрывается на конкурсе по результатам ЕНТ, и вход в конкурс
 * закрыт пороговым баллом: ниже порога заявку просто не принимают, каким бы
 * сильным ни было остальное портфолио. Это качественно иное ограничение, чем
 * «у вуза есть стипендии», поэтому fundingAvailable его не выражает.
 *
 * Поле опционально осознанно: у зарубежных программ такого механизма нет,
 * у автономных организаций образования (Nazarbayev University) — свой
 * собственный грант вне государственного конкурса. Оптональность делает
 * изменение схемы аддитивным: старые записи в БД остаются валидными.
 */
export interface GrantTrack {
  /** Пороговый балл ЕНТ для допуска к конкурсу, шкала 0–140. */
  entThreshold: number;
  /** Откуда взят порог и что он означает — показывается пользователю. */
  note: string;
}
...
export interface ProgramCosts {
  ...
  fundingNote?: string;
  /** Государственный грант РК, если программа участвует в конкурсе. */
  grant?: GrantTrack;
}
...
/**
 * Вывод по допуску к грантовому конкурсу.
 *
 * Сознательно категориальный и сознательно узкий: это оценка ДОПУСКА
 * к конкурсу по пороговому баллу, а не предсказание его исхода. Грант
 * разыгрывается между всеми допущенными, и МНВО отдельно предупреждает,
 * что пороговый балл гранта не гарантирует.
 */
export type GrantLikelihood = "reliable" | "competitive" | "unlikely";

export interface GrantOutlook {
  /** Порог программы, с которым сравнивали. */
  entThreshold: number;
  /** Балл абитуриента из анкеты. */
  entScore: number;
  /** Балл минус порог: отрицательное значение — к конкурсу не допускают. */
  gap: number;
  likelihood: GrantLikelihood;
  /** Одна фраза для под-бейджа в интерфейсе. */
  note: string;
}
...
export interface FactorScore {
  ...
  detail: string;
  /**
   * Сырое сравнение по грантовому конкурсу — только у фактора budget и только
   * для казахстанских программ с грантовым треком. Восьмого фактора не заводим:
   * грант меняет цену варианта, то есть это часть бюджета, а не новая ось.
   */
  grant?: GrantOutlook;
}
...
/**
 * Идентификатор правила проверки реальности. Стабилен: по нему UI и AI-слой
 * сопоставляют находку с её детерминированным оригиналом.
 */
export type RealityCheckId =
  | "field-vs-subjects"
  | "budget-vs-field"
  | "language-vs-plan"
  | "track-vs-geography"
...
/** Один выход из конфликта. Решение остаётся за абитуриентом, не за продуктом. */
export interface RealityCheckResolution {
  /** Стабильный ключ внутри находки — защита от подмены вариантов в AI-слое. */
  id: string;
  title: string;
  detail: string;
}
...
/**
 * Противоречие между заявленным приоритетом и собственными вводными профиля.
 * Не пробел (чего-то не хватает), а расхождение: два ответа анкеты тянут
 * в разные стороны, и абитуриенту нужно выбрать, какой из них главный.
 */
export interface RealityCheckFinding {
  id: RealityCheckId;
  /** Короткая формулировка расхождения — заголовок в интерфейсе. */
  title: string;
  /** Что именно с чем расходится, с цифрами и словами из самой анкеты. */
  conflict: string;
  /** 2–3 конкретных выхода. */
  resolutions: RealityCheckResolution[];
}
...
export interface Diagnosis {
  ...
  gaps: DiagnosisItem[];
  /** Конфликты между приоритетами и вводными. Пусто — значит расхождений нет. */
  realityChecks: RealityCheckFinding[];
